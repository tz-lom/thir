...
#define THIR_EXTENSION_INCLUDE(Ext, Class, Type) BOOST_PP_STRINGIZE(THIR_EXTENSION_IDENT(./extensions/)THIR_EXTENSION_IDENT(Ext)THIR_EXTENSION_IDENT(/)BOOST_PP_CAT(Class,BOOST_PP_CAT(_,Type)).hpp)
...
#define THIR_PRIV_GEN_CLASS_CPP_I(Records) \
    ...
    const ::std::size_t (THIR_NS::SerializedData::headerSizes)[] = {0 BOOST_PP_SEQ_FOR_EACH_I(THIR_PRIV_GEN_STATIC_HEADER_SIZE,_,Records) };\
    const ::std::size_t (THIR_NS::SerializedData::staticSizes)[] = {0 BOOST_PP_SEQ_FOR_EACH_I(THIR_PRIV_GEN_STATIC_STATIC_SIZE,_,Records) };\
    const ::std::size_t (THIR_NS::SerializedData::__LastType) = BOOST_PP_SEQ_FOLD_LEFT( THIR_PRIV_GEN_STATIC_LAST_ID , 0, Records)::ID ;
